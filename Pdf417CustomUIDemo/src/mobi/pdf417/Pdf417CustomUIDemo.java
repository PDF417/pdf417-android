...
public class Pdf417CustomUIDemo extends Activity {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == MY_REQUEST_CODE && resultCode == BaseBarcodeActivity.RESULT_OK) {
			 ...
			 * barcode elements. Each barcode element contains byte array with
			 * raw data for this element and the type of that byte array. The
			 * type can be either ElementType.TEXT_DATA or
			 * ElementType.BYTE_DATA. TEXT_DATA means that bytes in that element
			 * represent text and can be converted to string. BYTE_DATA meand
			 * that bytes in that element don't represent anything in particular
			 * and you should decide of to use them.
			 * 
			 * Of course, both BYTE_DATA and TEXT_DATA fields can be converted
			 * to string (this is actually done in library for string result
			 * obtained in barcodeData variable).
			 */
