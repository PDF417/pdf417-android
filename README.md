...
```
dependencies {
    compile 'com.microblink:pdf417.mobi:4.5.1'
}
```
...
_PDF417.mobi_ does not have any additional dependencies.
...
With this method you can set a [RecognizerViewEventListener](https://pdf417.github.io/pdf417-android/com/microblink/view/recognition/RecognizerViewEventListener.html) which will be notified when certain recognition events occur, such as when object has been detected.
...
With this method you can query `RecognizerView` if it is capable of recognizing [Android Bitmaps](https://developer.android.com/reference/android/graphics/Bitmap.html) or [Image objects](https://pdf417.github.io/pdf417-android/com/microblink/image/Image.html). `RecognizerView` is capable of that if it has been started or resumed.
...
You can use this method to define [image listener](https://pdf417.github.io/pdf417-android/com/microblink/image/ImageListener.html) that will obtain images that are currently being processed by the native library. For more information and example implementation, see [Using ImageListener to obtain images that are being processed](#imageListener)
...
1. First, you need to obtain reference to [Recognizer singleton](https://pdf417.github.io/pdf417-android/com/microblink/directApi/Recognizer.html).
...
Generic settings affect all enabled recognizers and the whole recognition process. The complete reference can be found in [javadoc](https://pdf417.github.io/pdf417-android/com/microblink/recognizers/settings/GenericRecognizerSettings.html). Here is the list of methods that are most relevant:
...
To activate PDF417 recognizer, you need to create a [Pdf417RecognizerSettings](https://pdf417.github.io/pdf417-android/com/microblink/recognizers/barcode/pdf417/Pdf417RecognizerSettings.html) and add it to `RecognizerSettings` array. You can do this using following code snippet:
...
PDF417 recognizer produces [Pdf417ScanResult](https://pdf417.github.io/pdf417-android/com/microblink/recognizers/barcode/pdf417/Pdf417ScanResult.html). You can use `instanceof` operator to check if element in results array is instance of `Pdf417ScanResult` class. See the following snippet for an example:
...
To activate USDL recognizer, you need to create [USDLRecognizerSettings](https://pdf417.github.io/pdf417-android/com/microblink/recognizers/barcode/usdl/USDLRecognizerSettings.html) and add it to `RecognizerSettings` array. You can do this using following code snippet:
...
To activate Bardecoder recognizer, you need to create a [BarDecoderRecognizerSettings](https://pdf417.github.io/pdf417-android/com/microblink/recognizers/barcode/bardecoder/BarDecoderRecognizerSettings.html) and add it to `RecognizerSettings` array. You can do this using following code snippet:
...
##### `getBarcodeType()`
This method will return a [BarcodeType](https://pdf417.github.io/pdf417-android/com/microblink/recognizers/barcode/BarcodeType.html) enum that defines the type of barcode scanned.
...
To activate ZXing recognizer, you need to create [ZXingRecognizerSettings](https://pdf417.github.io/pdf417-android/com/microblink/recognizers/barcode/zxing/ZXingRecognizerSettings.html) and add it to `RecognizerSettings` array. You can do this using the following code snippet:
...
ZXing recognizer produces [ZXingScanResult](https://pdf417.github.io/pdf417-android/com/microblink/recognizers/barcode/zxing/ZXingScanResult.html). You can use `instanceof` operator to check if element in results array is instance of `ZXingScanResult` class. See the following snippet for example:
